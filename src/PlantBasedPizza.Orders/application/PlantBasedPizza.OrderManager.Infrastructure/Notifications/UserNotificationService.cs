...
public class UserNotificationService(IHubContext<OrderNotificationsHub> hub) : IUserNotificationService
{
    ...
    public async Task NotifyPaymentFailed(string customerIdentifier, string orderIdentifier)
    {
        await hub.Clients.User(customerIdentifier).SendAsync("paymentFailed", orderIdentifier);
    }
